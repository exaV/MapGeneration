...
public class ScalingMixer extends AbstractVideoFX implements IScalingFrameSource {
	private static FXParameter[] GAINS = new FXParameter[0]; 
	...
	public ScalingMixer(IVideoFrameSource ... sources) {
		...
		init(FTS_RGBA8_RGB8, sources);
		setSize(super.getWidth(), super.getHeight());
	}

	private static FXParameter[] makeParams(int n) {
		int count = GAINS.length;
		if(n > GAINS.length)
			GAINS = Arrays.copyOf(GAINS, n);
		for(int i = count; i < n; i++)
			GAINS[i] = new FXParameter("g" + i, "Gain channel " + i, 0, 1, i == 0 ? 1 : 0);
