...
public class ImageScaler {
	public final static int NORMALIZE      = 1;
	public final static int DETECT_NO_DATA = 2;
			...
			int blendARGB)
	{
		...
		do {
			...
			if(type == BufferedImage.TYPE_INT_ARGB) {
				g2.setRenderingHint(RenderingHints.KEY_ALPHA_INTERPOLATION, higherQuality ? 
		...
		if(isCompatible(src, dst))
			...
			try {
				...
				case DataBuffer.TYPE_FLOAT :
					...
					// special treatment for ArcMap terrain output (32bit float tiffs) : hardcoded linear mapping to USHORT
					// the g.drawImage() way results in wrong scaling, no idea why [dec]
					if(ddb.getDataType() == DataBuffer.TYPE_USHORT && 
							sdb.getDataType() == DataBuffer.TYPE_FLOAT && 
							dst.getNumXTiles() == src.getNumXTiles() && dst.getNumYTiles() == dst.getNumYTiles()) 
					{
						...
						for(int y = src.getNumYTiles(); --y >= 0;) {
							for(int x = src.getNumXTiles(); --x >= 0;) {
								...
								if(normalize) {
									if(detectNoData) {
										for(int i = 0; i < tmp.length; i++) {
											if(tmp[i] == noData) tmp[i] = min;
											else				 tmp[i] = (float)Math.rint((tmp[i] - min) / range * USHORTMAX);
										}
									} else {
										for(int i = 0; i < tmp.length; i++)
	...
	private static boolean isCompatible(BufferedImage src, BufferedImage dst) {
		return	isCompatible(src.getSampleModel(), dst.getSampleModel())
				&& isCompatible(src.getColorModel(), dst.getColorModel());
	}
	...
	private static boolean isCompatible(ColorModel src,	ColorModel dst) {
		return src.hasAlpha() == dst.hasAlpha() 
				&& isCompatible(src.getColorSpace(), dst.getColorSpace());
	}
